...
def _():
    ...
    In this homework, you will finetune a pretrained model (of the _exact_ same model code you developed in the previous assignment, just trained for longer) for chat training, using both supervised finetuning (SFT) and direct preference optimization (DPO).
    """)
    return
...
def generate(model, prompt_tokens, tokenizer, eot_token=None, temp=0.7, max_tokens=500, verbose=True):
    ...
    Inputs:
        ...
        eot_token: int - index of token at which to stop generation
...
def _():
    mo.md(r"""
    Using the pretrained model above, in this part, you'll implement code to tune it for chat.  For this purpose, we're using a subset of the [Ultrachat 200k](https://huggingface.co/datasets/HuggingFaceH4/ultrachat_200k) dataset, a common dataset used for this kind of finetuning.  The subset of these chats included in the homework just filter the original data to only include those conversations that tokenize to 1024 tokens or fewer (the sequence length for our model.)
    """)
    return
...
def _():
    ...
    First, implement a function that converts a set of ultrachat-style messages to a text format with user and assistant tags.  Specifically, Ultrachat uses a message format like the following
    ```json
    [
        {
            'role': 'user',
...
@app.cell(hide_code=True)
def _():
    submit_convert_to_chat_format_button = mo.ui.run_button(label="submit `convert_to_chat_format`")
...
def _(submit_convert_to_chat_format_button):
    mugrade.submit_tests(convert_to_chat_format) if submit_convert_to_chat_format_button.value else None
    return
...
@app.cell(hide_code=True)
def _():
    submit_pretokenize_chat_button = mo.ui.run_button(label="submit `pretokenize_chat`")
...
def _(submit_pretokenize_chat_button):
    mugrade.submit_tests(pretokenize_chat) if submit_pretokenize_chat_button.value else None
    return
...
def _(submit_get_loss_mask_button):
    mugrade.submit_tests(get_loss_mask) if submit_get_loss_mask_button.value else None
    return
...
def _(submit_DataLoaderChat_button):
    mugrade.submit_tests(DataLoaderChat) if submit_DataLoaderChat_button.value else None
    return
...
def _():
    ...
    y[mask]
    ```
    will return a 1D tensor _only_ of the entries in `y` corresponding to `True` entries in `mask`.  The same can be used to index based upon only the leading dimensions of a higher-order tensor.  You can see this e.g., in the following code:
    ```python
    A = torch.randn(3,4,5)
    mask = torch.randn(3,4) > 0
    print(A[mask])
    ```
    which will be a `n` by 5 dimensional tensor, where `n` is the total number of `True` entries in mask.
...
def _(submit_train_llm_chat_button):
    mugrade.submit_tests(train_llm_chat) if submit_train_llm_chat_button.value else None
    return
...
def _(eval_llm_chat, submit_eval_llm_chat_button):
    mugrade.submit_tests(eval_llm_chat) if submit_eval_llm_chat_button.value else None
    return
...
@app.cell
def _():
    # Uncomment to generate from trained chat model
    # prompt = "<USER>What is the capital of the United States?</USER><ASSISTANT>"
    # generate(model, tokenizer.encode(prompt, allowed_special="all"), tokenizer, eot_token=50300, temp=0.4);
    pass
    return
...
def _():
    ...
    For DPO, we will need the ability to compute the sum of log probabilities (i.e., negative cross entropy losses), not just averaged over the entire batch, but for each element in the batch separately.  Notationally, we will use the convention of computing the log probability instead of negating the cross-entropy loss, because this is more common in presentation of DPO, and it's helpful in better understanding the connection between the two perspectives.
...
def _():
    ...
    The softplus function acts as a softer version of a ReLU-like function, and captures the intuition that we want to minimize the term inside the DPO loss only when the inner term is positive, i.e., the probability of the negative completion is larger than the probability of the positive one.
    ...
    For efficiency and to ensure that you aren't actually modifying the reference model, you should ensure that all use of the reference model is done within a `torch.no_grad()` block, similar to how you implemented this in the optimizer.
    """)
    return
...
def _():
    ...
    Finally, we can put all these functions together to train a DPO model.  We will start by initializing the data loaders and model.  You can directly use the model you trained above as a starting point, but for ease of use, we recommend that you use a version of the model that we fine-tuned using the entire data set above, stored in `model_chat.pth`.  To start, you can train a small version on CPU.
    """)
    return
...
def _(eval_llm_dpo, submit_eval_llm_dpo_button):
    mugrade.submit_tests(eval_llm_dpo) if submit_eval_llm_dpo_button.value else None
    return
...
@app.cell
def _():
    # Uncomment to set up data loaders for large DPO run (requires GPU)
    # loader_neg = DataLoaderChat("ultrachat_neg_tokenized.json", 1024, 16, tokenizer, device="cuda")
    # loader_pos = DataLoaderChat("ultrachat_pos_tokenized.json", 1024, 16, tokenizer, device="cuda")
    # model.load_weights("model_chat.pth") # comment out if you want to use your own model
    # model.float().cuda();
...
@app.cell
def _():
    # Uncomment to test some prompt generation from full DPO model (requires GPU)
    # prompt = "<USER>Write a poem about flowers.</USER><ASSISTANT>"
    # generate(model, tokenizer.encode(prompt, allowed_special="all"), tokenizer, eot_token=50300, temp=0.4);
    pass
    return
